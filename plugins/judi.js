...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    try {
        ...
        let randomkamu = `${Math.floor(Math.random() * 95)}`.trim() //hehe Biar Seimbang
        let Aku = (randomaku * 1)
        ...
        if (global.db.data.users[m.sender].money >= count * 1) {
            ...
            await m.reply('*Jangan Pernah Berjudi. Ingat Kata Kata Di Lukisan Warnet Simulator*') //kemrenn
            if (Aku > Kamu) {
                ...
                global.db.data.users[m.sender].money += count * 1
                conn.reply(m.chat, `aku roll:${Aku}\nKamu Roll: ${Kamu}\n\nKita *JODOH*, Kamu Mendapatkan ${count * 1} Money`.trim(), m)
            }
