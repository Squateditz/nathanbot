let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    let who
    if (m.isGroup) who = arlgs[1] ? args[1] : m.chat
    else who = args[1]

    var harinya = 86400000 * args[0]
    var now = new Date() * 1
    if (now < global.db.data.chats[who].expired) global.db.data.chats[who].expired += harinya
    else global.db.data.chats[who].expired = now + jumlahHari
    m.reply(`Berhasil Menetapkan Hari Untuk Grup Ini Selama ${args[0]} Hari.\n\nHitung Mundur : ${msToDate(global.db.data.chats[who].expired - now)}`)
}
